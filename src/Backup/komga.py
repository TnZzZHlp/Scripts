...
KOMGA_CONFIG_PATH = "C:/Users/Administrator/Desktop/komga/config"
...
def log(message):
    ...
    if isinstance(log_folder, str) and os.path.exists(log_folder):
        ...
        # 记录日志
        with open(
            os.path.join(log_folder, "backup", "komga.txt"), "w", encoding="utf-8"
        ) as log_file:
...
def main():
    ...
    try:
        ...
        # 复制文件到临时文件夹
        log(f"正在复制 {KOMGA_CONFIG_PATH} 到 {tmp_folder}")
        if os.path.exists(KOMGA_CONFIG_PATH):
            for item in os.listdir(KOMGA_CONFIG_PATH):
                s = os.path.join(KOMGA_CONFIG_PATH, item)
            ...
            log(f"源路径不存在: {KOMGA_CONFIG_PATH}")
            sys.exit(1)
        ...
        # 删除日志文件夹
        logs_path = os.path.join(tmp_folder, "logs")
        if os.path.exists(logs_path):
            log(f"正在删除日志文件夹: {logs_path}")
